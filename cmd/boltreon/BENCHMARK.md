...
本文档介绍如何使用 `redis-benchmark` 工具对 Boltreon 服务器进行压力测试。
...
2. **启动 Boltreon 服务器**
   ```bash
   cd /Users/lbp/IdeaProjects/Boltreon
...
在启动 Boltreon 时，可以调整 BadgerDB 的参数（需要修改代码）：
...
1. **数据持久化**: Boltreon 使用 BadgerDB 进行持久化，写入性能可能低于纯内存的 Redis
2. **首次运行**: 第一次运行可能较慢，因为需要初始化数据库
3. **磁盘 I/O**: 确保有足够的磁盘空间和良好的 I/O 性能
4. **内存使用**: BadgerDB 会使用内存缓存，注意监控内存使用情况
5. **并发安全**: Boltreon 使用 BadgerDB 的事务机制，确保并发安全
